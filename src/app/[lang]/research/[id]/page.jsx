import { getResearch } from "@/services/topics";
import styles from "./page.module.css";
import { Relacionados } from "@/components/relacionados/Relacionados";
import { getDictionary } from "@/dictionary";

export default async function Page({ params }) {
  const lang = await getDictionary(params.lang);
  const pesquisa = lang.projects.projetos.find((news) => news.id === params.id);
  return (
    <div className={styles.main_div}>
      <main className={styles.main}>
        <div className={styles.img_div}>
          <img
            src={pesquisa?.img}
            alt="headline_image"
            className={styles.img}
          />
        </div>
        <div className={styles.title_container}>
          <div className={styles.divider}></div>
          <h2 className={styles.title}>{pesquisa?.title}</h2>
        </div>
        <section className={styles.author_container}>
          <h3
            className={styles.author_name}
          >{`${pesquisa?.date} - ${pesquisa?.status}`}</h3>
        </section>
        <section className={styles.text_body}>{pesquisa.body}</section>
      </main>
      <Relacionados
        title={lang.relacionados.title}
        articles={lang.relacionados.related}
      />
    </div>
  );
}
